...
## 6월 12일
css는 다른 거랑 비교했을 때 큰 차이가 없으니 개념 설명 생략  
사용법을 익히는게 빠르다  
### styled
![화면 캡처 2024-06-12 112343](https://github.com/madx1102/react1-1/assets/162391761/fbb97994-7e15-435c-8ff6-85bb75e90378)  
내용 정리를 위해 첨가한 임시 이미지 나중에 삭제할 예정
...
리액트에서는 합성을 사용해 특수화를 구현  
범용적으로 쓸 수 있는 컴포넌트를 만들고 이를 특수한 목적으로 사용하는 합성 방식 등이 있음  

![화면 캡처 2024-06-11 101413](https://github.com/madx1102/react1-1/assets/162391761/aa6048f3-10f5-4ff8-ad36-2246a2d5c3ff)  
...
리액트는 상속보다는 합성을 사용  
### 컨텍스트
리액트 컴포넌트 사이에서 데이터를 기존의 프롭스를 통해 전달하는 방식 대신 컴포넌트 트리를 통해 바로 컴포넌트에 전달하는 새로운 방식을 제공한다  
이 것을 통해 어떤 컴포넌트라도 쉽게 데이터에 접근 할 수 있다  
컨텍스트를 사용하면 데이터를 필요로 하는 컴포넌트에 곧바로 데이터를 전달할 수 있다. 

### 컨텍스트를 사용할 시기
자주 사용하는 데이터를 바로 전달해야 할 때 사용하면 코드의 가독성과 생산성을 늘릴 수 있다  
### 컨텍스트를 사용할 때 고려할 점 
컨텍스트와 컴포넌트가 연동되면 재사용성이 떨어지기 때문에 다른 레벨의 많은 컴포넌트가 데이터를 필요로 하는 경우가 아니면  
프롭스를 통해 데이터를 전달하는 컴포넌트 합성 방법이 더 적합하다  
### ㅇㅇㅇ
ㅇㅇㅇ
## 6월 5일
예비군 감
...
value prop은 넣되 자유롭게 입력하게 만들고 싶으면 값이 null 혹은 undefined를 넣으면 된다.  
...
![화면 캡처 2024-05-22 114000](https://github.com/madx1102/react1-1/assets/162391761/ea2f58a2-9d70-4f54-8d79-b76a3da30727)   
### 폼
일반적으로 사용자로부터 입력을 받기 위한 양식에서 많이 사용됨
### 제어 컴포넌트
